...
// SduCoorCustomRequirementViewer: mirrors AdviserCustomRequirementViewer but scoped for SduCoor oversight.
// SduCoor can approve or request revision for a specific organization's custom requirement submission.
export default function SduCoorCustomRequirementViewer({
  requirementKey,
  title,
  selectedOrg,
}) {
  ...
  useEffect(() => {
    ...
    async function fetchSubmission() {
      ...
      try {
        ...
        console.warn(
          "SduCoorCustomRequirementViewer: submission fetch failed",
          e.message,
        );
      } finally {
  ...
  async function updateStatus(next, note) {
    ...
    if (!submissionId) {
      console.warn(
        "SduCoorCustomRequirementViewer: missing submission id for status update",
      );
      return;
    }
    try {
  ...
  const statusStyles = {
    ...
    SduCoorApproved: "bg-indigo-50 text-indigo-700 border border-indigo-200",
    ...
    >
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 flex items-center gap-3">
                ...
                {submission?.status && (
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-medium ${statusStyles[submission.status] || "bg-gray-100 text-gray-600 border"}`}
                  >
                    {submission.status}
                  </span>
                )}
              ...
              <p className="text-xs uppercase tracking-wide text-gray-500">
                Custom Requirement • Key: {requirementKey}
              </p>
            </div>
          </div>
          {/* SduCoor actions: only after AdviserApproved */}
          {submission && submission.status === "AdviserApproved" && (
            <div className="flex gap-2">
              ...
              >
                <CheckCircle2 className="w-4 h-4" /> Mark Sdu Coordinator
                Approved
              </button>
              <button
                onClick={() => {
        ...
        <p className="text-sm text-gray-600">
          Sdu Coordinator oversight for custom accreditation requirement. Review
          and approve or request clarification/revision.
        </p>
        {loading && (
        ...
        {!loading && !submission && (
          <div className="p-4 rounded-md bg-gray-50 border text-sm text-gray-600">
            No submission yet.
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            {previewError && (
              <div className="p-3 bg-red-50 border border-red-200 rounded text-xs text-red-700">
                Preview failed. File may be missing or in legacy path; ask
                student to re-upload.
              </div>
            )}
      ...
      {showApproveModal && (
        <DecisionModal
          ...
          onConfirm={async () => {
            ...
            setSubmitting(true);
            await updateStatus("Sdu Coordinator Approved", decisionNote);
          }}
        />
      )}
      {showRevisionModal && (
...
function DecisionModal({
  ...
  const title =
    type === "approve" ? "Mark SduCoor Approved" : "Request Revision";
  const actionLabel =
    type === "approve"
      ? "Confirm Sdu Coordinator Approval"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-fadeIn">
        ...
        <div className="px-6 py-5 space-y-4">
          {type === "approve" ? (
            <p className="text-sm text-gray-600">
              You are about to mark this document as Approved. Optionally leave
              a note for the log.
            </p>
          ) : (
            <p className="text-sm text-gray-600">
              Provide clear revision instructions. This will log as a status
              change to Rejected (Revision Requested).
            </p>
          )}
          <div>
            ...
            <textarea
              ...
              placeholder={
                type === "approve"
                  ? "e.g. Meets all criteria."
                  : "e.g. Please clarify section 2 and attach signed roster."
              }
