...
import { SduCoorPresident } from "./individual-accreditation/sdu-coor-accreditation-president";
import { SduCoorAccreditationDocument } from "./individual-accreditation/sdu-coor-accreditation-documents";
...
import { SduCoorProposedPlan } from "./individual-accreditation/sdu-coor-accreditation-proposed-plan";
import SduCoorCustomRequirementViewer from "./individual-accreditation/sdu-coor-custom-requirement-viewer";
...
export function OrgHome({
  ...
  // Visible accreditation requirements (template + custom) for dynamic SduCoor tabs
  const [visibleRequirements, setVisibleRequirements] = useState(null); // null = loading, [] = loaded none/error
  // SduCoor header tabs: start with templates then append custom requirement titles
  ...
  // Fetch visible requirements once (global list) to build SduCoor custom tabs
  useEffect(() => {
    let ignore = false;
    async function fetchVisible() {
      try {
        ...
        console.warn(
          "SduCoor OrgHome: failed to fetch visible requirements",
          e?.message,
        );
      }
    }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <header className="relative flex h-14 w-full border-b border-gray-200 overflow-x-auto custom-scroll whitespace-nowrap flex-shrink-0 bg-background">
        ...
        <span
          className="absolute bottom-0 h-[2px] bg-primary transition-all duration-500 ease-in-out"
      ...
      >
        ...
        {tab === "President Information" && (
          <div className="w-full h-full">
            <SduCoorPresident selectedOrg={displayOrg} />
          </div>
        )}
        {tab === "Roster of Members" && (
          <SduCoorRosterData selectedOrg={displayOrg} />
        )}

        {tab === "Documents" && (
          <div className="w-full h-full">
            <SduCoorAccreditationDocument selectedOrg={displayOrg} />
          </div>
        )}
        {tab === "Proposed Action Plan" && (
          <div className="w-full h-full">
            <SduCoorProposedPlan selectedOrg={displayOrg} />
          </div>
        )}
        ...
        {customHeadertabs.map((cr) =>
          tab === cr.label ? (
            <div key={cr.key} className="w-full h-full">
              <SduCoorCustomRequirementViewer
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="h-18 border-b bg-background flex items-center justify-between px-6 flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Monitor organization accreditation details
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-sm font-medium">
              {org?.orgName || baseOrg?.orgName || "Organization"}
            </p>
            <p className="text-xs text-muted-foreground">
              {org?.orgAcronym || baseOrg?.orgAcronym || ""}
            </p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        {!loading && !error && <AccreditationTable data={orgData} />}
      </div>
    </div>
  );
}
...
export function OrgActivities({ org, activities, user }) {
  ...
  return <SduCoorProposalConduct orgData={org} user={user} />;
}
...
export function OrgAccomplishments({ org, user, displayOrg }) {
  return <SduCoorAccomplishmentReport orgData={displayOrg} user={user} />;
}
